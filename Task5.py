...
def collect_info(directory_path):
    ...
    for entry in os.listdir(directory_path):
        ...
        logging.info(f' {file_info.name} |\
                        {file_info.extension if file_info.extension else"N/A"} |\
                        {"Directory" if file_info.is_directory else"File"}|\
                        {file_info.parent_directory}')

def main():
    """Основная функция для обработки командной строки и сбора информации."""
    parser = ArgumentParser(description="Сбор информации о содержимом директори и изапись в лог.")
